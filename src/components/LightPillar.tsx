...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  useEffect(() => {
    ...
    // Clamp pixel ratio to keep GPU load reasonable
    ...
    const handleVisibility = () => {
      ...
      isActive = currentlyVisible;
      if (currentlyVisible && !rafRef.current) {
        lastTime = performance.now();
        rafRef.current = requestAnimationFrame(animate);
      } else if (!currentlyVisible && rafRef.current) {
    ...
    const handleResize = () => {
      ...
      resizeTimeout = window.setTimeout(() => {
        ...
        const newWidth = Math.max(
          1,
          Math.floor(containerRef.current.clientWidth * renderScale)
        );
        const newHeight = Math.max(
          1,
          Math.floor(containerRef.current.clientHeight * renderScale)
        );
