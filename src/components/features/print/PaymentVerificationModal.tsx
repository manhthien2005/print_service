...
import { PaymentMethod, PagePackage } from '@/data/buyPagesMock';
import { paymentMethods } from '@/data/buyPagesMock';
...
interface PaymentVerificationModalProps {
  ...
  selectedPackage: PagePackage | null;
  customPages: number;
}
...
export function PaymentVerificationModal({
  ...
  const calculateTotal = () => {
    if (purchaseType === 'package' && selectedPackage) {
      return selectedPackage.priceVnd;
    }
    if (purchaseType === 'custom' && customPages > 0) {
      return customPages * 500; // 500 VND per page
    }
    return 0;
  };
  ...
  const getTotalPages = () => {
    if (purchaseType === 'package' && selectedPackage) {
      return selectedPackage.pages + selectedPackage.bonusPages;
    }
    return customPages;
  };
          ...
          <div className="space-y-2">
            ...
            {purchaseType === 'package' && selectedPackage && (
              <>
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold text-slate-900 dark:text-white">
                    {selectedPackage.pages.toLocaleString('vi-VN')} trang
                  </span>
                </div>
                {selectedPackage.bonusPages > 0 && (
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-600 dark:text-white/70">
                      Trang bonus:
                    </span>
                    <span className="font-semibold text-green-600 dark:text-green-400">
                      +{selectedPackage.bonusPages} trang
                    </span>
                  </div>
                )}
              </>
            ...
            {purchaseType === 'custom' && (
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-slate-900 dark:text-white">
                  {customPages.toLocaleString('vi-VN')} trang
                </span>
              </div>
            )}
            <div className="mt-3 border-t border-slate-200 pt-3 dark:border-white/10">
              <div className="flex justify-between">
                <span className="text-base font-semibold text-slate-900 dark:text-white">
                  Tổng trang nhận được:
                </span>
                <span className="text-lg font-bold text-blue-600 dark:text-blue-400">
                  {totalPages.toLocaleString('vi-VN')} trang
                </span>
              </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 text-sm text-amber-800 dark:text-amber-200">
              ...
              <div className="mt-1">
                ...
                thanh toán thành công, trang in sẽ được cộng vào tài khoản của
                bạn ngay lập tức.
              </div>
            </div>
          </div>
