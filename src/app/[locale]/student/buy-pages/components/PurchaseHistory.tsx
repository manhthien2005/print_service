...
type StatusFilterValue = 'all' | RechargeStatus;
type PaymentFilterValue = 'all' | PaymentMethod;
type SortColumn = 'date' | 'amount' | 'price' | 'status' | 'pages' | null;
...
function StatusBadge({ status }: { status: RechargeStatus }) {
  ...
  const label: Record<RechargeStatus, string> = {
    ...
    >
      ...
      <div className="col-span-2">
        <div className="text-sm text-slate-600 dark:text-white/70">
          {formatDate(item.rechargedAt)}
        </div>
      </div>

      <div className="col-span-2">
        <div className="text-sm font-semibold text-slate-800 dark:text-white">
          {item.packageName || 'Nạp tùy chỉnh'}
        </div>
        {item.packageId && (
      ...
      <div className="col-span-1">
        ...
        {item.bonusAmountVnd > 0 && (
          <div className="text-xs text-green-600 dark:text-green-400">
            +{formatPrice(item.bonusAmountVnd)} bonus
          </div>
        )}
        <div className="text-xs text-slate-500 dark:text-white/60">
          Tổng: {formatPrice(item.totalAmountVnd)}
        </div>
      </div>
...
export function PurchaseHistory() {
  ...
  const [selected, setSelected] = useState<RechargeHistoryItem | null>(null);
  ...
  const filtered = useMemo(() => {
    return rechargeHistoryMock.filter(item => {
      const matchesStatus = status === 'all' || item.status === status;
      const matchesPayment =
        paymentMethod === 'all' || item.paymentMethod === paymentMethod;

      const purchased = new Date(item.rechargedAt);
    ...
    data.sort((a, b) => {
      ...
      switch (sortColumn) {
        case 'date':
          return (
            direction *
            (new Date(a.rechargedAt).getTime() -
              new Date(b.rechargedAt).getTime())
          );
        case 'amount':
          return direction * (a.totalAmountVnd - b.totalAmountVnd);
        case 'price':
          return direction * (a.amountVnd - b.amountVnd);
        case 'status':
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <SummaryCard
          ...
          useCountUp
          countUpValue={rechargeSummaryMock.totalRecharges}
          caption="Đã hoàn thành"
        />
        <SummaryCard
          title="Tổng tiền nạp"
          value={formatPrice(rechargeSummaryMock.totalAmountRecharged)}
          caption={`+${formatPrice(rechargeSummaryMock.totalBonusAmount)} bonus`}
        />
        <SummaryCard
          title="Tổng chi tiêu"
          value={formatPrice(rechargeSummaryMock.totalSpentVnd)}
          caption="Tất cả giao dịch thành công"
        />
        <SummaryCard
          title="Tiền bonus"
          value={formatPrice(rechargeSummaryMock.totalBonusAmount)}
          caption="Từ các gói nạp"
        />
      </div>
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <div className="text-sm text-slate-500 dark:text-white/60">
              Tìm kiếm, lọc và sắp xếp lịch sử nạp tiền
            </div>
          </div>
              ...
              <div className="rounded-xl border border-slate-200/70 bg-slate-50/70 p-4 dark:border-white/10 dark:bg-white/5">
                ...
                <div className="mt-2 space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-slate-900 dark:text-white">
                      {selected.packageName || 'Nạp tùy chỉnh'}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-600 dark:text-white/70">
                    ...
                    <span className="font-semibold text-slate-900 dark:text-white">
                      {formatDate(selected.rechargedAt)}
                    </span>
                  </div>
              ...
              <div className="rounded-xl border border-slate-200/70 bg-slate-50/70 p-4 dark:border-white/10 dark:bg-white/5">
                ...
                <div className="mt-2 space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-slate-900 dark:text-white">
                      {formatPrice(selected.amountVnd)}
                    </span>
                  </div>
                  {selected.bonusAmountVnd > 0 && (
                    <div className="flex justify-between">
                      <span className="text-slate-600 dark:text-white/70">
                        Tiền bonus:
                      </span>
                      <span className="font-semibold text-green-600 dark:text-green-400">
                        +{formatPrice(selected.bonusAmountVnd)}
                      </span>
                    </div>
                  )}
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-blue-600 dark:text-blue-400">
                      {formatPrice(selected.totalAmountVnd)}
                    </span>
                  </div>
