...
export default async function StudentPrintersPage({
  params,
}: {
  ...
  const t = await import(`@/locales/${locale}/pages.json`).then(m => m.default.student.printers);
      ...
      <div className="relative z-10 mx-auto flex min-h-screen max-w-7xl flex-col px-6 py-10">
        ...
        <div className="flex-1">
          <div className="rounded-2xl border border-white/10 bg-white/5 p-8 backdrop-blur-md">
            <p className="text-white/60">Printers Info content coming soon...</p>
          </div>
        </div>
      </div>
