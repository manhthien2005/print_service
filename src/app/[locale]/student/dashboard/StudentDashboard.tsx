...
import { FileIcon } from '@/components/features/print/FileIcon';
import {
  Card,
  CardContent,
  CardDescription,
...
export default function StudentDashboard({
  ...
  const greetingText =
    studentCopy.greeting?.replace(
      '{name}',
      studentProfileSummaryMock.fullName
    ) ?? `Xin chào, ${studentProfileSummaryMock.fullName}`;
      ...
      <div className="space-y-4">
        <div className="flex flex-col gap-2">
          <h1 className="text-4xl font-bold text-slate-900 dark:text-white">
            {studentCopy.sectionLabel ?? 'Trang của sinh viên'}
          </h1>
          <p className="text-base text-slate-600 dark:text-white/70">
            {studentCopy.subtext ?? 'Thống kê và theo dõi'}
          </p>
        </div>

        <div className="grid items-stretch gap-4 xl:grid-cols-[1.6fr,1fr]">
          <Card className="relative h-full overflow-hidden border-slate-200/70 bg-gradient-to-br from-sky-50 via-white to-indigo-50 shadow-lg backdrop-blur dark:border-white/10 dark:from-white/10 dark:via-white/5 dark:to-white/0">
            ...
            <CardHeader className="relative space-y-1">
              <CardTitle className="text-3xl font-bold text-slate-900 dark:text-white">
                {studentCopy.heroTitle ?? greetingText}
              </CardTitle>
              <CardDescription className="text-base text-slate-600 dark:text-white/70">
                {studentCopy.heroSubtitle ?? 'Thống kê và theo dõi'}
              </CardDescription>
            </CardHeader>
            <CardContent className="relative mt-2 grid gap-4 lg:grid-cols-2">
              <div className="space-y-4 rounded-2xl border border-slate-200/70 bg-white/80 p-5 shadow-sm dark:border-white/10 dark:bg-white/5">
                {/* Số dư tổng */}
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-slate-500 dark:text-white/70">
                      {studentCopy.balance?.title ?? 'Số dư tổng'}
                    </p>
                    ...
                    <p className="mt-1 text-sm text-slate-500 dark:text-white/60">
                      {studentCopy.balance?.resetLabel
                        ? studentCopy.balance.resetLabel.replace(
                            '{date}',
                            studentStatsMock.quotaReset
                          )
                        : `Làm mới vào ${studentStatsMock.quotaReset}`}
                    </p>
                  </div>
                  <span className="rounded-full bg-sky-100 px-3 py-1 text-xs font-semibold text-sky-700 dark:bg-sky-500/15 dark:text-sky-200">
                    {studentCopy.balance?.badge ?? 'Tổng'}
                  </span>
                </div>

                {/* Số dư tặng */}
                <div className="rounded-xl border border-slate-200/70 bg-white/70 px-4 py-3 dark:border-white/10 dark:bg-white/5">
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  <div className="rounded-xl border border-slate-200/70 bg-white/70 px-3 py-3 text-sm dark:border-white/10 dark:bg-white/5">
                    ...
                    {studentStatsMock.jobsChangePercent !== 0 && (
                      <p
                        className={`mt-1 text-xs ${studentStatsMock.jobsChangePercent > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}
                      >
                        {studentStatsMock.jobsChangePercent > 0 ? '+' : ''}
                        {studentStatsMock.jobsChangePercent}% so với tháng trước
                      </p>
                    )}
                  </div>
                  <div className="rounded-xl border border-slate-200/70 bg-white/70 px-3 py-3 text-sm dark:border-white/10 dark:bg-white/5">
                    ...
                    {studentStatsMock.pagesChangePercent !== 0 && (
                      <p
                        className={`mt-1 text-xs ${studentStatsMock.pagesChangePercent > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}
                      >
                        {studentStatsMock.pagesChangePercent > 0 ? '+' : ''}
              ...
              <div className="space-y-4 rounded-2xl border border-slate-200/70 bg-white/80 p-5 text-sm shadow-sm dark:border-white/10 dark:bg-white/5">
                {/* Hạn mức được tặng - Progress bar */}
                ...
                {/* Quick Actions */}
                <div className="rounded-xl border border-dashed border-slate-200/80 px-3 py-3 dark:border-white/15">
                  <div className="mt-3 flex flex-wrap gap-2">
                    <Link href={withLocale('/student/print')}>
                      <Button
                        size="sm"
                        className="bg-slate-900 text-white dark:bg-white dark:text-slate-900"
                      >
                        {studentCopy.ctaPrimary ?? 'In tài liệu'}
                      </Button>
                    </Link>
                    <Link href={withLocale('/student/buy-pages')}>
                      <Button
                        size="sm"
                        variant="secondary"
                        className="border-slate-200 text-slate-700 dark:border-white/15 dark:bg-white/5 dark:text-white"
                      >
                        {studentCopy.ctaSecondary ?? 'Mua thêm trang'}
                      </Button>
                    </Link>
                  </div>
                </div>
