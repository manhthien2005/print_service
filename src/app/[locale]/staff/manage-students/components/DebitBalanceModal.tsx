...
interface DebitBalanceModalProps {
  isOpen: boolean;
  onClose: () => void;
  userId: string | null;
  userName?: string;
...
export const DebitBalanceModal: React.FC<DebitBalanceModalProps> = ({
  isOpen,
  onClose,
  userId,
  userName,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const response = await debitBalance.mutateAsync({
        userId,
        data: {
  ...
  const newBalance =
    currentBalance !== undefined && formData.amount > 0
        ...
        <div className="space-y-6">
          ...
          {currentBalance !== undefined && (
            <div className="rounded-lg border border-rose-200 bg-rose-50 p-4 dark:border-rose-800 dark:bg-rose-900/20">
              ...
              {newBalance !== undefined && newBalance >= 0 && (
                <div className="mt-2 text-sm font-medium text-rose-700 dark:text-rose-300">
                  {t('balanceAfter')}:{' '}
                  <span className="font-bold">
                    {new Intl.NumberFormat('vi-VN', {
                      style: 'currency',
          ...
          <div className="flex justify-end gap-3 border-t border-slate-200/50 pt-4 dark:border-white/10">
            ...
            <Button
              type="submit"
              disabled={
                isLoading || (newBalance !== undefined && newBalance < 0)
              }
