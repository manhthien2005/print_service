...
import {
  usePageDiscountPackages,
  useCreatePageDiscountPackage,
  useUpdatePageDiscountPackage,
  useDeletePageDiscountPackage,
  type PageDiscountPackageResponse,
  type CreatePageDiscountPackageRequest,
...
export function PageDiscountSection() {
  const t = useTranslations('staff.configuration.pageDiscount');
  const { data, isLoading, error } = usePageDiscountPackages();
  const createMutation = useCreatePageDiscountPackage();
  const updateMutation = useUpdatePageDiscountPackage();
  const deleteMutation = useDeletePageDiscountPackage();
  const [isCreateModalOpen, setIsCreateModalOpen] = useState(false);
  const [editingItem, setEditingItem] =
    useState<PageDiscountPackageResponse | null>(null);
  const [formData, setFormData] = useState<CreatePageDiscountPackageRequest>({
    packageName: '',
    minPages: 0,
  ...
  const handleEdit = (item: PageDiscountPackageResponse) => {
    setEditingItem(item);
    setFormData({
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-2 flex gap-4 text-sm">
                      <span className="text-slate-600 dark:text-white/70">
                        {t('minimum')}: {item.minPages} {t('pages')}
                      </span>
                      <span className="text-slate-600 dark:text-white/70">
                        {t('discount')}:{' '}
                        {(item.discountPercentage * 100).toFixed(1)}%
                      </span>
                    </div>
                    <div
        ...
        <div className="space-y-4 p-6">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder={t('minPagesPlaceholder')}
              className="w-full"
              min="0"
              step="1"
            />
