...
import {
  useAdminPrintHistory,
  useAdminPrintHistoryStats,
  type AdminPrintHistoryItem,
} from '@/lib/api/services/adminPrintHistory';
import { PrintHistoryStats } from './PrintHistoryStats';
import { PrintHistoryFiltersComponent } from './PrintHistoryFilters';
import { PrintHistoryTable } from './PrintHistoryTable';
import { PrintHistoryDetailModal } from './PrintHistoryDetailModal';
import { PrintHistoryStatsSkeleton } from './PrintHistorySkeleton';
import type { PrintHistoryFilters, SortColumn, SortDirection } from '../types';

const PAGE_SIZE = 20;

export function ManagePrintHistoryContent() {
  ...
  const [selectedItem, setSelectedItem] =
    useState<AdminPrintHistoryItem | null>(null);
  const [isDetailModalOpen, setIsDetailModalOpen] = useState(false);

  // Filters state
  const [filters, setFilters] = useState<PrintHistoryFilters>({
    page: 0,
    limit: PAGE_SIZE,
    sortBy: 'createdAt',
    sortDirection: 'desc',
  ...
  const apiFilters = useMemo(() => {
    const apiFilterParams: Parameters<typeof useAdminPrintHistory>[0] = {
    ...
    if (filters.printerId) apiFilterParams.printerId = filters.printerId;
    ...
    if (filters.status && filters.status !== 'all')
      apiFilterParams.status = filters.status;
    if (filters.fileType && filters.fileType !== 'all')
      apiFilterParams.fileType = filters.fileType;
    if (filters.paymentMethod && filters.paymentMethod !== 'all')
      apiFilterParams.paymentMethod = filters.paymentMethod;
  ...
  const statsFilters = useMemo(() => {
    const statsParams: Parameters<typeof useAdminPrintHistoryStats>[0] = {};
  ...
  const {
    ...
    error: historyError,
  } = useAdminPrintHistory(apiFilters);

  const {
    data: statsResponse,
    isLoading: isLoadingStats,
    error: statsError,
  } = useAdminPrintHistoryStats(statsFilters);
  ...
  // Handle item click
  const handleItemClick = (item: AdminPrintHistoryItem) => {
    setSelectedItem(item);
    setIsDetailModalOpen(true);
  };

  // Handle filter changes
  const handleFiltersChange = (newFilters: PrintHistoryFilters) => {
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Statistics Cards */}
      {isLoadingStats ? (
        <PrintHistoryStatsSkeleton />
      ) : (
        <PrintHistoryStats stats={stats} isLoading={isLoadingStats} />
      )}
