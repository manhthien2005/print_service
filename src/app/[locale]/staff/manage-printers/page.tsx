...
import ManagePrintersContent from '@/components/features/ManagePrintersContent';

export function generateStaticParams() {
  return locales.map(locale => ({ locale }));
}

export default async function ManagePrintersPage({
  params,
}: {
  ...
  const t = await import(`@/locales/${locale}/pages.json`).then(
    m => m.default.staff.managePrinters
  );
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto flex min-h-screen max-w-7xl flex-col px-6 py-10 pb-24">
        ...
        <ManagePrintersContent translations={t} />
      </div>
